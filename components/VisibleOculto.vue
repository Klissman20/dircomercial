<template>
  <div class="text-center flex justify-center items-center gap-2">
    ...
    <button
      class="py-1 hover:shadow px-6 bg-opacity-80 hover:bg-opacity-100 scale-95 text-white hover:scale-100 duration-75 ease-in-out rounded-md border"
      @click="$emit('update:modelValue', !modelValue)"
      :class="modelValue ? 'bg-green-500' : 'bg-red-700'"
    >
      {{ !modelValue ? "Oculto" : "Visible" }}
    </button>
  </div>
</template>
